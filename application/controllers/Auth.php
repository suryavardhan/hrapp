...
class Auth extends CI_Controller
{
	...
	function register()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user('1',
		...
		if (!$this->tank_auth->is_logged_in(FALSE)) {							// not logged in or activated
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->change_email(
					...
					$data['site_name']	= $this->config->item('portal_name', 'plus_hr');
					$data['activation_period'] = $this->config->item('email_activation_expire', 'tank_auth') / 3600;
	...
	function _send_email($type, $email, &$data)
	{
		...
		$this->email->from($this->config->item('webmaster_email', 'tank_auth'), $this->config->item('portal_name', 'plus_hr'));
		$this->email->reply_to($this->config->item('webmaster_email', 'tank_auth'), $this->config->item('portal_name', 'plus_hr'));
		$this->email->to($email);
        ...
        //Send information to administration also
